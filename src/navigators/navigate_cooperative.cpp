...
#include <memory>
#include <limits>
#include "nav2_cooperative/navigate_cooperative.hpp"

namespace nav2_cooperative
{

bool
NavigateCooperative::configure(
  ...
  std::shared_ptr<nav2_util::OdomSmoother> odom_smoother)
{
  ...
  goal_sub_ = node->create_subscription<geometry_msgs::msg::PoseStamped>(
    ...
    std::bind(&NavigateCooperative::onGoalPoseReceived, this, std::placeholders::_1));
  return true;
}

std::string
NavigateCooperative::getDefaultBTFilepath(
  rclcpp_lifecycle::LifecycleNode::WeakPtr parent_node)
{
  ...
  if (!node->has_parameter("default_nav_to_pose_bt_xml")) {
    std::string pkg_share_dir =
      ament_index_cpp::get_package_share_directory("nav2_cooperative");
...
bool
NavigateCooperative::goalReceived(ActionT::Goal::ConstSharedPtr goal)
{
...
void
NavigateCooperative::goalCompleted(
...
void
NavigateCooperative::onPreempt(ActionT::Goal::ConstSharedPtr goal)
{
...
void
NavigateCooperative::initializeGoalPose(ActionT::Goal::ConstSharedPtr goal)
{
  RCLCPP_INFO(
...
void
NavigateCooperative::onGoalPoseReceived(const geometry_msgs::msg::PoseStamped::SharedPtr pose)
{
  ActionT::Goal goal;
  goal.pose = *pose;
...
PLUGINLIB_EXPORT_CLASS(
  nav2_cooperative::NavigateCooperative,
